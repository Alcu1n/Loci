...
- Use [Natural Earth Admin 0 – Countries at 1:10m](https://www.naturalearthdata.com/downloads/10m-cultural-vectors/), pinned to repository tag `v5.1.2` and commit `f1890d9f152c896d250a77557a5751a93d494776`. The generated resource retains the higher-detail coastlines and islands needed by offline point-in-polygon lookup.
...
- Use the official [`cities1000.zip`](https://download.geonames.org/export/dump/) dump as the source. Retain every valid populated-place row instead of applying an additional population filter, including every `PPLC` and `PPLA*` administrative seat. Preserve both the source name and ASCII name in the generated SQLite database.
...
- Because the official URLs roll forward, the exact 2026-07-12 `cities1000` and `admin1CodesASCII` inputs are vendored under `scripts/offline-geodata-sources/`. Clean-checkout regeneration reads those immutable copies and verifies their recorded hashes; the official URLs remain provenance and update sources.

### License and redistribution

GeoNames applies [CC BY 4.0](https://download.geonames.org/export/dump/) to the dump. The [CC BY 4.0 legal code](https://creativecommons.org/licenses/by/4.0/legalcode.en) expressly permits reproducing and sharing the material and producing, reproducing, and sharing adapted material (section 2); it also covers extraction and reuse of database contents (section 4). Therefore, redistributing a transformed SQLite database inside the app is permitted, provided the attribution conditions are met.
...
- say that Loci transformed and indexed the source data;
...
> Contains GeoNames data, licensed under CC BY 4.0. Data has been transformed
> and indexed for offline lookup.

## Reproducible generation pipeline

Implement one deterministic script and run it explicitly when updating data:

1. Download the pinned Natural Earth 10m v5.1.2 GeoJSON, read the vendored GeoNames `cities1000` and `admin1CodesASCII` snapshot, and verify every SHA-256 hash recorded in `scripts/build-offline-geodata.mjs`.
...
5. Emit a manifest containing generation/download dates, URLs, source hashes, row counts, and output hashes. Fail if fewer than 250 country features or 165,000 populated places survive validation, or if the expanded resources exceed their safety budgets.
